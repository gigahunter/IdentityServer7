...
namespace IdentityServer.IntegrationTests.Endpoints.Authorize
{
    public class JwtRequestAuthorizeTests
    {
        ...
        public JwtRequestAuthorizeTests(ITestOutputHelper output)
        {
            PersistentGrantSerializer.OverrideSerializer = true;
        ...
        public async Task authorize_should_accept_complex_objects_in_request_object()
        {
            ...
            /*
            var handler = new JwtSecurityTokenHandler();
            var token = handler.ReadJwtToken(requestJwt);
            */
            var extra = Parameters.FromObject(new
            {
                request = requestJwt
            });
            ...
            var str = _mockPipeline.LoginRequest.Parameters["someObj"];
            Console.WriteLine(str);
            var someObj2 = JsonConvert.DeserializeObject(str, someObj.GetType());
